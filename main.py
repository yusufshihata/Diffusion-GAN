...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, required=True, help="Usage: --mode [train, inference]"
    )
    parser.add_argument(
        "--dataset",
        type=str,
        required=True,
        help="Usage: --dataset Dataset to do training on [cifar, mnist].",
    )
    ...
    if args.mode == "train":
        parser.add_argument(
            ...
            type=int,
            required=False,
            help="Usage: --epochs Number of epochs to train the model for.",
        )
        parser.add_argument(
            "--optimizer",
            type=str,
            required=False,
            help="Usage: --optimizer [SGD, Adam].",
        )
        parser.add_argument(
            "--glr",
            type=float,
            required=False,
            help="Usage: --glr Generator Training Learning Rate.",
        )
        parser.add_argument(
            "--dlr",
            type=float,
            required=False,
            help="Usage: --dlr Discriminator Training Learning Rate.",
        )
        ...
        gen = Generator(
            config["training"]["latent_dim"], config["training"]["img_channels"]
        ).to(device)
